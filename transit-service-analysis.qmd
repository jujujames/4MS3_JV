...
params:
  ...
  gtfs_files: 
    ...
    - "UP-GTFS.zip"
    - "GO-GTFS.zip"
  ghs_built_c_file: "GHS_BUILT_C_FUN_E2018_GLOBE_R2023A_54009_10_V1_0_R4_C12.tif"
  wgs84_utm_code: "32617"
  census_dataset: "CA21"
...
# Load and filter each GTFS file using a date-based method
list_city_gtfs_filtered <- lapply(gtfs_paths_raw, function(path) {
  ...
  if ("calendar" %in% names(gtfs_data)) {
    if (analysis_day %in% names(gtfs_data$calendar)) {
      # Ensure service_id is character
  ...
  if ("calendar_dates" %in% names(gtfs_data)) {
    # Ensure service_id is character and exception_type is integer
